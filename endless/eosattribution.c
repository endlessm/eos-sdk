  ...
  for (count = 0; count < num_images; count++)
    {
      ...
      const gchar *original_uri = NULL, *license = NULL, *license_uri = NULL,
        ...
        *comment = NULL, *resource_path = NULL, *resource_uri = NULL;
...
#define READ_MEMBER_IF_PRESENT(member_name, read_func, storage) \
  if (strv_index (members, member_name) != -1) \
    { \
      ...
      READ_MEMBER_IF_PRESENT("resource_uri", string_value, resource_uri)
...
#undef READ_MEMBER_IF_PRESENT
      ...
      if (resource_path == NULL && resource_uri == NULL)
        {
          g_warning ("Image must have one of the following specified: resource_path, resource_uri");
          continue;
        }

      const char *debug_name = resource_path ? resource_path : resource_uri;
      ...
      if (resource_path)
        ...
        {
          g_autoptr(GFile) file = g_file_new_for_uri (resource_uri);
