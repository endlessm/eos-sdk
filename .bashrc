...
if [ "$color_prompt" = yes ]; then
    PS1='${debian_chroot:+($debian_chroot)}$(__git_ps1 "\[\e[0;33m\](%s) \[\e[m\]")\[\e[0;35m\]\w\$ \[\e[m\]'
else
    PS1='${debian_chroot:+($debian_chroot)}$(__git_ps1 "(%s) ")\w\$ '
fi
...
if [ -z "$UNDER_JHBUILD" ]; then
  insults=('gluttonous' 'glutenous' 'flatulent' 'boil-brained' 'onion-eyed' 'overly hirsute' 'craven' 'raving' 'rancid' 'rank' 'cantankerous' 'pulchritudinous' 'moldy' 'molting' 'goat-kneading' 'MATLAB-loving' 'saucy' 'spicy' 'defenestrating' 'git-hating' 'teenage mutant ninja')
  num_insults=${#insults[*]}
